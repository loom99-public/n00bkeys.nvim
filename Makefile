...
# Timeout after 400s to prevent indefinite hangs (288 test cases take ~350s in practice)
test:
	make deps
	...
	if [ -z "$$TIMEOUT_CMD" ]; then \
		...
		$$TIMEOUT_CMD 400s nvim --headless --noplugin -u ./scripts/minimal_init.lua \
			-c "lua MiniTest.run({ execute = { reporter = MiniTest.gen_reporter.stdout({ group_depth = 2 }) } })" \
			|| (echo "ERROR: Tests timed out after 400 seconds or failed" && exit 1); \
	fi
